...
export async function GET(req: NextRequest) {
  try {
    ...
    if (profile?.country !== 'GB') {
      return new NextResponse('UK ETS verification is only available for GB accounts', { status: 403 });
    }

    // Load installation (ownership check via profile_id)
    ...
    // Load emissions for the year (account-level, for context)
    ...
    if (format === 'json') {
      ...
      return new NextResponse(body, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="uk-ets-verification-${installation.permit_number}-${year}.json"`,
        },
      });
    }
    ...
    if (format === 'csv') {
      const rows = [
        ...
        ['Free Allocation (UKAs)', verification?.free_allocation ?? ''],
        ['Purchased Allowances (UKAs)', verification?.purchased_allowances ?? ''],
        ['Surrendered Allowances (UKAs)', verification?.surrendered_allowances ?? ''],
        ['Net Position (UKAs)', verification ? String(safe(verification.free_allocation) + safe(verification.purchased_allowances) - safe(verification.surrendered_allowances)) : ''],
      ...
      return new NextResponse(csv, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="uk-ets-verification-${installation.permit_number}-${year}.csv"`,
        },
      });
    }
    ...
    const section = (title: string) => {
      y -= 14;
      page.drawRectangle({ x: ML, y: y - 3, width: CW, height: 16, color: rgb(0.95, 0.97, 0.99) });
      text(title.toUpperCase(), ML + 6, y + 5, 7.5, bold, rgb(0.3, 0.45, 0.65));
      y -= 8;
      line(y);
      y -= 10;
    };
    ...
    // Header band
    page.drawRectangle({ x: 0, y: PH - 52, width: PW, height: 52, color: rgb(0.08, 0.28, 0.22) });
    text('UK EMISSIONS TRADING SCHEME', ML, PH - 22, 9, bold, rgb(0.55, 0.9, 0.7));
    text('Verification Report', ML, PH - 36, 13, bold, rgb(1, 1, 1));
    text(`Reporting Year: ${year}`, MR - 90, PH - 22, 8, font, rgb(0.75, 0.95, 0.85));
    text(`Generated: ${new Date().toLocaleDateString('en-GB')}`, MR - 90, PH - 36, 8, font, rgb(0.75, 0.95, 0.85));
    ...
    if (!verification) {
      ...
      const statusLabel = (verification.status as string).replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    ...
    if (!verification) {
      ...
      row('Free Allocation', `${fmt(fa)} UKAs`);
      row('Purchased Allowances', `${fmt(pa)} UKAs`);
      row('Surrendered Allowances', `${fmt(sa)} UKAs`);

      // Net position row with colour
      y -= 2;
      line(y + 13);
      const posColor = position >= 0 ? rgb(0.05, 0.5, 0.25) : rgb(0.75, 0.1, 0.1);
      text('Net Position', ML, y, 8.5, font, rgb(0.45, 0.45, 0.45));
      const posStr = `${position >= 0 ? '+' : ''}${fmt(position)} UKAs`;
      ...
      if (position < 0) {
        ...
        text(`${fmt(Math.abs(position))} UKA shortfall x \u00a3100/tonne = \u00a3${fmt(penalty)} exposure`, ML + 6, y - 10, 8, font, rgb(0.55, 0.1, 0.1));
        y -= 34;
      }
    }
    ...
    text('This document is generated from Greenio. It is not a substitute for official DESNZ/EA compliance submissions.', ML, 20, 7, font, rgb(0.55, 0.55, 0.55));

    const pdfBytes = await pdfDoc.save();
    return new NextResponse(new Uint8Array(pdfBytes), {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="uk-ets-verification-${installation.permit_number}-${year}.pdf"`,
      },
    });

  } catch (err: any) {
    console.error('[uk-ets-verification]', err);
