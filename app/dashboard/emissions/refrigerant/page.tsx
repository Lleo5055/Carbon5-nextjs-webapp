...
export default function RefrigerantInsightsPage({
  searchParams,
}: {
  ...
  const [data, setData] = useState<RefrigerantInsightsData>({
    months: [],
    totalKg: 0,
    totalCo2eKg: 0,
    lastMonth: null,
  ...
  useEffect(() => {
    async function fetchRefrigerantInsights() {
      ...
      let baseMonths: DashboardMonth[] = rawData.map((row: any) => {
        ...
        const refrigerantCode: string | null = row.refrigerant_code ?? 'GENERIC_HFC';
      ...
      baseMonths.sort((a, b) => new Date(a.monthLabel).getTime() - new Date(b.monthLabel).getTime());
      ...
      const insightsMonths: InsightsMonth[] = periodMonths.map((m) => ({
        ...
        refrigerantKg: m.refrigerantKg,
        refrigerantCode: m.refrigerantCode ?? 'GENERIC_HFC',
        co2eKg: calcRefrigerantCo2e(m.refrigerantKg, m.refrigerantCode ?? 'GENERIC_HFC'),
      }));
      ...
      const totalRef = periodMonths.reduce(
        (s, m) => s + calcRefrigerantCo2e(m.refrigerantKg, m.refrigerantCode ?? 'GENERIC_HFC'),
        0
      );
      ...
      setData({
        ...
        lastMonth,
        prevMonth,
        shareOfFootprintPercent: normalisedShares.refrigerant,
      });

      setLoading(false);
    }

    fetchRefrigerantInsights();
  }, [period]);
  ...
  const { months, totalKg, totalCo2eKg, lastMonth, prevMonth, shareOfFootprintPercent } = data;
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-500">Loading refrigerant insights...</div>;
  }
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 space-y-6">
        ...
        {!hasData ? (
          <section className="rounded-xl bg-white border p-8 text-center shadow">
            <p className="text-sm font-medium text-slate-800">No refrigerant data yet.</p>
            <p className="text-xs text-slate-500 mt-1">
              Once you log refrigerant top-ups or losses, this view will show how they affect your footprint and leak
              risk.
            </p>
          </section>
        ) : (
            ...
            <section className="grid gap-4 md:grid-cols-3">
              <div className="rounded-xl bg-white border p-4 shadow-sm">
                ...
                <p className="mt-1 text-[11px] text-slate-500">
                  From {formatNumber(totalKg)} kg of refrigerant recorded in this period.
                </p>
              </div>
              ...
              <div className="rounded-xl bg-white border p-4 shadow-sm">
                ...
                <p className="mt-1 text-[11px] text-slate-500">
                  Portion of your total emissions in this period that comes from refrigerant.
                </p>
              </div>
              ...
              <div className="rounded-xl bg-white border p-4 shadow-sm">
                ...
                <p className="mt-1 text-[11px] text-slate-500">
                  Change in refrigerant-related CO₂e between the last two reported months.
                </p>
              </div>
            </section>
            ...
            <section className="grid gap-4 md:grid-cols-[minmax(0,2fr)_minmax(0,1.4fr)]">
              <article className="rounded-xl bg-white border p-6 shadow flex flex-col gap-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <p className="text-[11px] text-slate-500">Refrigerant CO₂e per month</p>
                </div>

                <div className="flex items-center gap-4">
                  <Sparkline values={chartValues} />
                  <p className="text-[11px] text-slate-600">
                    Spikes usually indicate leak events or large top-ups. Use this trend to confirm whether actions are
                    reducing losses.
                  </p>
                </div>
                  ...
                  <table className="min-w-full text-xs">
                    <thead>
                      <tr className="text-slate-500 border-b border-slate-100">
                        ...
                        <th className="py-1.5 text-right font-medium">Refrigerant (kg)</th>
                    ...
                    <tbody>
                      {months.map((m, idx) => (
                        <tr key={m.monthLabel} className="border-b border-slate-50 last:border-0">
                          ...
                          <td className="py-1.5 text-right text-slate-700">{m.refrigerantKg.toFixed(2)}</td>
                          <td className="py-1.5 text-right text-slate-700">{m.refrigerantCode ?? 'GENERIC'}</td>
              ...
              <aside className="rounded-xl bg-white border p-6 shadow flex flex-col gap-3">
                ...
                <p className="text-[11px] text-slate-500 mb-1">
                  When refrigerant dominates your footprint, even small leaks matter. These actions focus on visibility and
                  prevention.
                </p>

                <ul className="grid gap-2 text-[11px]">
                  <li className="border rounded-lg px-3 py-2 bg-slate-50">
                    <p className="font-semibold text-slate-900 mb-0.5">Lock in a leak-check schedule</p>
                    <p className="text-slate-600">
                      Identify critical units and schedule regular inspections. Replace old or non-compliant equipment.
                    </p>
                  </li>

                  <li className="border rounded-lg px-3 py-2 bg-slate-50">
                    <p className="font-semibold text-slate-900 mb-0.5">Track top-ups immediately</p>
                    <p className="text-slate-600">
                      Log any refrigerant added to systems promptly to understand whether emissions are rising.
                    </p>
                  </li>

                  <li className="border rounded-lg px-3 py-2 bg-slate-50">
                    <p className="font-semibold text-slate-900 mb-0.5">Prioritize high-GWP refrigerants</p>
                    <p className="text-slate-600">
                      If possible, replace high global warming potential refrigerants with lower-impact alternatives.
                    </p>
                  </li>
                </ul>
              </aside>
