...
export default function EUInstallationDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (instErr || !inst) { router.push('/dashboard/eu-ets/installations'); return; }
  ...
  async function handleDownloadReport(format: 'pdf' | 'csv' | 'json') {
    ...
    try {
      ...
      if (!session) return;
      const url = `/api/eu-ets-verification?userId=${session.user.id}&token=${session.access_token}&installationId=${id}&year=${selectedYear}&format=${format}`;
      ...
      a.download = `eu-ets-verification-${permit}-${selectedYear}.${ext}`;
      a.click();
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/dashboard/eu-ets/installations" className="inline-flex items-center gap-1.5 h-[32px] px-4 rounded-full bg-slate-900 text-white text-xs font-medium hover:bg-slate-800">
              ← Installations
            </Link>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="rounded-xl bg-white border border-slate-200 p-4 shadow">
            ...
            <p className={`text-sm font-semibold mt-1 ${position >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
              {ver ? `${position >= 0 ? '+' : ''}${position.toLocaleString()} EUAs` : '—'}
            </p>
          </div>
          <div className="rounded-xl bg-white border border-slate-200 p-4 shadow">
        ...
        {tab === 'overview' && (
          <section className="rounded-xl bg-white border border-slate-200 shadow divide-y divide-slate-100">
            ...
            <div className="px-5 py-3 flex items-center gap-2">
              <Link href="/dashboard/eu-ets/installations" className="text-xs text-blue-600 hover:underline underline-offset-2">Edit installation →</Link>
            </div>
          </section>
        )}
            ...
            <form onSubmit={handleSaveVerification} className="rounded-xl bg-white border border-slate-200 p-6 shadow space-y-5">
              ...
              <div className="border-t border-slate-100 pt-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-xs font-medium text-slate-700">Free allocation (EUAs)</label>
                  ...
                  <div>
                    <label className="text-xs font-medium text-slate-700">Purchased allowances (EUAs)</label>
                  ...
                  <div>
                    <label className="text-xs font-medium text-slate-700">Surrendered allowances (EUAs)</label>
          ...
          <section className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-xl bg-white border border-slate-200 p-5 shadow">
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">EUAs granted free</p>
              </div>
              <div className="rounded-xl bg-white border border-slate-200 p-5 shadow">
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">EUAs bought on market</p>
              </div>
              <div className="rounded-xl bg-white border border-slate-200 p-5 shadow">
                ...
                <p className="text-[11px] text-slate-400 mt-0.5">EUAs already surrendered</p>
              </div>
            </div>
            ...
            <div className="rounded-xl bg-white border border-slate-200 p-5 shadow space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between py-2 border-b border-slate-100">
                  ...
                  <span className="text-sm font-semibold text-slate-900 tabular-nums">{(freeAlloc + purchased).toLocaleString()} EUAs</span>
                </div>
                <div className="flex items-center justify-between py-2 border-b border-slate-100">
                  <span className="text-xs text-slate-600">Surrendered</span>
                  <span className="text-sm font-semibold text-slate-900 tabular-nums">{surrendered.toLocaleString()} EUAs</span>
                </div>
                <div className="flex items-center justify-between py-3">
                  ...
                  <span className={`text-lg font-bold tabular-nums ${position >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {position >= 0 ? '+' : ''}{position.toLocaleString()} EUAs
                  </span>
                </div>
              </div>
              ...
              {position < 0 && (
                <div className="rounded-lg bg-rose-50 border border-rose-100 px-4 py-3 space-y-1">
                  ...
                  <p className="text-xs text-rose-700">
                    {Math.abs(position).toLocaleString()} EUA shortfall × €100/tonne = <strong>€{penaltyRisk.toLocaleString()}</strong> penalty exposure
                  </p>
              ...
              {position >= 0 && ver && (
                <div className="rounded-lg bg-emerald-50 border border-emerald-100 px-4 py-3">
                  ...
                  <p className="text-xs text-emerald-700 mt-0.5">
                    {position.toLocaleString()} surplus EUAs can be banked for future years or sold on the secondary market.
                  </p>
                </div>
              )}
