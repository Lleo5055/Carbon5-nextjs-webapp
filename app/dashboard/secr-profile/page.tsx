...
type FormState = {
  secr_required: boolean;
  employee_count: string;
  annual_revenue: string;
  annual_output_units: string;
  energy_efficiency_actions: string;
...
function getCompletion(form: FormState): CompletionItem[] {
  return [
    { label: 'SECR applicability confirmed', done: form.secr_required },
    { label: 'Employee count entered', done: !!form.employee_count },
    { label: 'Annual revenue entered', done: !!form.annual_revenue },
    { label: 'Energy efficiency actions described', done: form.energy_efficiency_actions.trim().length > 20 },
    { label: 'Methodology confirmed', done: form.methodology_confirmed },
  ];
}

export default function SecrProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: profile } = await supabase
        .from('profiles')
        .select('country, secr_required, employee_count, annual_revenue, annual_output_units, energy_efficiency_actions, methodology_confirmed, renewable_energy_tariff, has_company_vehicles')
        .eq('id', user.id)
        .maybeSingle();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: err } = await supabase
      .from('profiles')
      .update({
        secr_required:             form.secr_required,
        ...
        energy_efficiency_actions: form.energy_efficiency_actions.trim() || null,
  ...
  const meetsRevenueThreshold = revenue >= 36_000_000;
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] uppercase tracking-widest text-blue-700 font-semibold bg-blue-50 border border-blue-100 rounded-full px-2 py-0.5">SECR</span>
          </div>
          <h1 className="text-2xl font-semibold text-slate-900">SECR Company Profile</h1>
          <p className="text-sm text-slate-600 mt-1">
            Streamlined Energy and Carbon Reporting, required for large UK companies. Complete this profile to generate SECR-compliant reports.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-xs text-blue-800 space-y-1">
          <p className="font-semibold">Who must report under SECR?</p>
          <p>Large UK companies incorporated under the Companies Act 2006 that meet at least two of: (1) 250+ employees, (2) £36M+ annual turnover, (3) £18M+ balance sheet total. Quoted companies must also include energy intensity ratios.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="rounded-xl bg-white border border-slate-200 p-6 shadow space-y-5">
            <h2 className="text-sm font-semibold text-slate-900">SECR applicability</h2>

            <label className="flex items-start gap-2 text-sm text-slate-700 cursor-pointer">
              <input
                type="checkbox"
                checked={form.secr_required}
                onChange={e => update('secr_required', e.target.checked)}
                className="mt-0.5 accent-blue-600"
              />
              <span>
                My company is required to comply with SECR (Streamlined Energy and Carbon Reporting).
                <span className="block text-xs text-slate-400 mt-0.5">Required for large UK companies (250+ employees or £36M+ turnover).</span>
              </span>
            </label>
              ...
              <div>
                ...
                {form.annual_revenue && (
                  <p className={`text-[11px] mt-0.5 font-medium ${meetsRevenueThreshold ? 'text-emerald-600' : 'text-slate-400'}`}>
                    {meetsRevenueThreshold ? '✓ Meets £36M+ threshold' : 'Below £36M threshold'}
                  </p>
                )}
              </div>
            </div>
            ...
            {(form.employee_count || form.annual_revenue) && (
              <div className={`rounded-lg px-4 py-3 text-xs ${likelyRequired ? 'bg-amber-50 border border-amber-100 text-amber-800' : 'bg-slate-50 border border-slate-200 text-slate-600'}`}>
                {likelyRequired
                  ? '⚠ Based on the figures entered, your company likely meets the SECR reporting threshold.'
                  : 'Based on the figures entered, your company may be below the mandatory SECR threshold. Voluntary reporting is still encouraged.'}
              </div>
            )}
          </div>

          {/* Section 2: Energy intensity */}
          <div className="rounded-xl bg-white border border-slate-200 p-6 shadow space-y-5">
            <h2 className="text-sm font-semibold text-slate-900">Energy intensity ratio</h2>
            <p className="text-xs text-slate-500">Quoted companies must disclose an energy intensity ratio. Enter your chosen denominator (e.g. units produced, revenue £, floor area m²).</p>
            <div>
              ...
              />
              <p className="text-[11px] text-slate-400 mt-0.5">Greenio will calculate tCO₂e per unit using this figure in your SECR report.</p>
            </div>
          </div>
          ...
          <div className="rounded-xl bg-white border border-slate-200 p-6 shadow space-y-4">
            ...
            <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
              ...
              <span className="text-[11px] text-slate-400 ml-1">(fuel use must be included in SECR scope)</span>
            </label>
            <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
              <input
                type="checkbox"
              ...
              <span className="text-[11px] text-slate-400 ml-1">(must be disclosed in SECR narrative)</span>
            </label>
          </div>

          {/* Section 4: Energy efficiency actions */}
          <div className="rounded-xl bg-white border border-slate-200 p-6 shadow space-y-4">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Energy efficiency actions</h2>
              <p className="text-xs text-slate-500 mt-1">Mandatory for SECR. Describe actions taken in the reporting year to improve energy efficiency (e.g. LED lighting upgrades, HVAC optimisation, insulation, smart metering).</p>
            </div>
            <textarea
              rows={5}
              value={form.energy_efficiency_actions}
              onChange={e => update('energy_efficiency_actions', e.target.value)}
              className="w-full rounded-lg border px-3 py-2 text-sm bg-white"
              placeholder="Describe energy efficiency measures implemented during the reporting year…"
            />
            <p className={`text-[11px] ${form.energy_efficiency_actions.trim().length > 20 ? 'text-emerald-600' : 'text-slate-400'}`}>
              {form.energy_efficiency_actions.trim().length > 20 ? '✓ Sufficient detail provided' : 'Please provide at least a brief description of actions taken.'}
            </p>
          </div>
          ...
          <div className="rounded-xl bg-white border border-slate-200 p-6 shadow space-y-4">
            ...
            <div className="rounded-lg bg-slate-50 border border-slate-200 p-3">
              <p className="text-xs text-slate-600 leading-relaxed">
                <strong>Methodology used for SECR reporting:</strong><br />
                Greenio uses geo-optimised emission factors aligned to BEIS/DEFRA official sources. Electricity uses location-based grid factors; fuel emissions use DEFRA kg CO₂e per litre values. Reporting aligns with SECR guidance and the GHG Protocol Corporate Standard.
              </p>
            </div>
            <label className="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
              <input
          ...
          <div className="flex items-center gap-3 pb-4">
            ...
            >
              {saving ? 'Saving…' : 'Save SECR profile'}
            </button>
            <Link href="/dashboard/uk-ets" className="text-xs text-blue-600 hover:underline underline-offset-2">
              Go to UK ETS Dashboard →
            </Link>
          </div>
        </form>
