...
export default function UKETSInstallationsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let err = null;
    if (editingId) {
      const { error: e } = await supabase.from('uk_ets_installations').update(payload).eq('id', editingId);
      err = e;
    } else {
      const { error: e } = await supabase.from('uk_ets_installations').insert(payload);
      err = e;
    }
    ...
    if (u) {
      const { data } = await supabase.from('uk_ets_installations').select('*').eq('profile_id', u.id).order('created_at', { ascending: false });
      setInstallations(data ?? []);
    }
  ...
  async function handleDelete(id: string) {
    const { error } = await supabase.from('uk_ets_installations').delete().eq('id', id);
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/dashboard/uk-ets" className="inline-flex items-center gap-1.5 h-[32px] px-4 rounded-full bg-slate-900 text-white text-xs font-medium hover:bg-slate-800">
              ← UK ETS
            </Link>
          </div>
          ...
          <section className="space-y-3">
            ...
            {installations.map(inst => (
              <article key={inst.id} className="rounded-xl bg-white border border-slate-200 p-5 shadow">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <Link href={`/dashboard/uk-ets/installations/${inst.id}`} className="text-sm font-semibold text-slate-900 hover:text-blue-700 underline underline-offset-2">
                ...
                <div className="mt-3">
                  <Link href={`/dashboard/uk-ets/installations/${inst.id}`} className="text-[11px] text-blue-600 hover:underline underline-offset-2">
